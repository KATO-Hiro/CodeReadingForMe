...
+ ユーザからデータを収集するために使う
  + 異なるフォールフィールドに入力して、データをリモートDBに送信する
...
+ 以下のサンプルは、個人手にはかなり肥大したコンポーネントという印象を受ける
  + React的には、これが普通な感じ?
  + パーツごとに分割して、もっとスマートに書けるはず
  + 属性が多すぎるのは危険な兆候では?
    + 確か、Qiitaでスマートに書くための方法が紹介されていたはず
...
+ 入力データのチェックとデータ形式の検証
+ 望ましいデータをユーザから得る + 悪意のあるユーザやデータを防ぐ
