...
+ local mutation
  + レンダリング中に、作成したばかりの変数やオブジェクトを変更してもまったく問題ない。
  + コンポーネントの小さな秘密みたいなもの
...
+ 関数型プログラミングは純粋さに大きく依存しているが、ある時点で、どこかで、何かを変えなければならない。それがプログラミングのポイントだ！画面の更新、アニメーションの開始、データの変更などは、副作用と呼ばれます。これらは、レンダリング中ではなく「サイド」で起こるものだ。

+ Reactでは、サイドエフェクトは通常イベントハンドラの中に属します。イベントハンドラとは、例えばボタンをクリックしたときなど、何らかのアクションを実行したときにReactが実行する関数のことです。イベントハンドラはコンポーネントの内部で定義されますが、レンダリング中には実行されません！そのため、イベントハンドラは純粋である必要はありません。

+ 他のすべてのオプションを使い果たして、サイドエフェクトに適切なイベントハンドラが見つからない場合でも、コンポーネントのuseEffectコールで、返されたJSXにイベントハンドラをアタッチすることができます。これは、レンダリングの後で、副作用が許可されているときに実行するようにReactに指示します。ただし、この方法は最後の手段にしてください。

+ 可能な限り、レンダリングだけでロジックを表現するようにしてください。そうすることで、驚くほど多くのことができるようになります！
